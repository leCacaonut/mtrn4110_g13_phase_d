...
int main(int argc, char **argv) {
  ...
  cout << "Start: " << startingPos[0] << "," << startingPos[1] << endl;
  ...
  printMap(cValues, hWalls, vWalls, startingPos, heading);
  ...
  #ifdef DEBUG
  if(path.size() > 0) cout << path.size() << " path(s) found" << endl;
  // print raw paths
  for(unsigned int i = 0; i < path.size(); ++i) {
    cout << i << ": " << path[i]->directionalPath << endl;
  }
  #endif
  ...
  for(unsigned int i = 0; i < path.size(); ++i) {
    ...
    #ifdef DEBUG
    cout << i + 1 << ": " << path[i]->instructionalPath << endl;
    cout << path[i]->expense << endl;
    #else
    cout << "--- Path " << i + 1 << " ---" << endl;
    #endif
    // print path on map
  ...
  cout << "--- Task 3 ---" << endl;
  #ifdef DEBUG
  cout << "Shortest path with least turns found!\n";
  ...
  #ifdef DEBUG
  cout << "ID: " << pathID << " expense: " << exp << endl;
  #endif
